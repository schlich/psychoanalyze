...
curves_sessions = sessions.merge(curves, left_index=True, right_index=True)
...
app.layout = html.Div([
    ...
    dcc.Graph(
        ...
        id='weber'),
    dbc.Input(id="curve-select", placeholder="Type a curve ID to isolate curve below:", type="number"),
    dcc.Graph(
...
def switch_mode(x_var, config):
    ...
    const_var_marks = {
        str(val): f'{val} {units}' for val in sorted(values)
    }
    value = counts.idxmax()
...
def display_selected_data(date_range, x_var, const_val, config, weber_data, threshold_data, curve_select):
